...
3. **Run the Release workflow** (`workflow_dispatch`). `changesets/action`
   opens a `chore: version packages` PR that bumps versions and regenerates
   `CHANGELOG.md`.
4. **Merge the version PR.** The action publishes to npm and creates GitHub
   Releases. Tags are `vX.Y.Z`, immutable, and never moved.
...
2. A migration page under `docs/migrations/` linked from the release notes.
