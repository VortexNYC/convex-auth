...
`AuthSignInForm` and `AuthSignUpForm` render standalone — they take values and callbacks as props, so they work in any React tree:

<Component path="sign-in-form" />

<Component path="sign-up-form" />
