...
- **Fix:** `updateSession` (refresh) now resolves the session's identity, so sessions minted through OAuth and passkeys refresh correctly. Previously the refresh path hardcoded a `password`/`native` identity lookup and failed for every other provider. `authSessions` gains an optional `identityId` column — all session-mint paths (password/OTP/2FA, OAuth callback, anonymous) now store it, and rotation/convergence propagate it to the new row. Sessions minted before the column existed resolve via the `identityId` claim already present in their JWT; a session with neither fails closed rather than guessing a provider.
